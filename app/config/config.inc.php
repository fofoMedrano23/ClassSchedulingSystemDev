<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "schedulingsystem");
if (!defined("PJ_PASS")) define("PJ_PASS", "schedulingsystem");
if (!defined("PJ_DB")) define("PJ_DB", "schedulingsystem");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/ClassSchedulingSystemDev/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "/opt/lampp/htdocs/ClassSchedulingSystemDev/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/ClassSchedulingSystemDev/");
if (!defined("PJ_SALT")) define("PJ_SALT", "DVGTUWQE");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>