			...
			if ($count > 0)
			{
				...
				<div class="clear_both">
					<form id="frmNotification" action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
						...
						<div id="tabs">
							<ul>
								<li><a href="#tabs-1"><?php __('tabToCustomers');?></a></li>
								<li><a href="#tabs-2"><?php __('tabToAdministrators');?></a></li>
							</ul>
							<div id="tabs-1">
								<?php
										...
										<tbody>
											...
											for ($i = 0; $i < $count; $i++)
											{
												if ($tpl['arr'][$i]['tab_id'] == 3 && (int) $tpl['arr'][$i]['is_visible'] === 1 && (strpos($tpl['arr'][$i]['key'], 'admin') === false) && (strpos($tpl['arr'][$i]['key'], 'email') > -1))
												{
													...
													if (in_array($tpl['arr'][$i]['key'], array('o_email_account', 'o_email_account_subject', 'o_email_account_message')))
													{
													...
													<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
														<td width="180" valign="top">
															...
															{
																?>
																<span class="fs10"><?php echo __('opt_' . $tpl['arr'][$i]['key'].'_text', true); ?></span>
																<?php
															} 
															?>
														</td>
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											<?php
											$client_notify_arr = __('client_sms', true);
											...
											<tr>
												<td><b><?php __('lblNotifications'); ?></b></td>
												<td>
													<select name="client_sms_notify" id="client_sms_notify" class="pj-form-field w300">
														<?php
														foreach ($client_notify_arr as $k => $v)
														{
											...
											for ($i = 0; $i < $count; $i++)
											{
												if ($tpl['arr'][$i]['tab_id'] == 5 && (int) $tpl['arr'][$i]['is_visible'] === 1 && (strpos($tpl['arr'][$i]['key'], 'admin') === false) && (strpos($tpl['arr'][$i]['key'], 'sms') > -1))
												{
													...
													{
														$rowClass = " boxClientSms boxClientSmsConfirmation";
													
													}
													if (in_array($tpl['arr'][$i]['key'], array('o_sms_payment_message')))
													{
														$rowClass = " boxClientSms boxClientSmsPayment";
							...
							<div id="tabs-2">
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											<?php
											$client_notify_arr = __('client_email', true);
											unset($client_notify_arr['account']);
											unset($client_notify_arr['forgot']);
											$key_arr = array('confirmation','payment','cancel');
											$client_notify_arr = pjUtil::sortArrayByArray($client_notify_arr, $key_arr);
											?>
											<tr>
												<td><b><?php __('lblNotifications'); ?></b></td>
											...
											for ($i = 0; $i < $count; $i++)
											{
												if (($tpl['arr'][$i]['tab_id'] == 4 && (int) $tpl['arr'][$i]['is_visible'] === 1 && strpos($tpl['arr'][$i]['key'], 'admin') >-1 && (strpos($tpl['arr'][$i]['key'], 'email') > -1)))
												{											
													$rowClass = NULL;
													$rowStyle = "display: none";
								...
								<fieldset class="fieldset white">
									...
									<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
										...
										<tbody>
											<?php
											$client_notify_arr = __('admin_sms', true);
											$key_arr = array('confirmation','payment');
											$client_notify_arr = pjUtil::sortArrayByArray($client_notify_arr, $key_arr);
											?>
											<tr>
												<td><b><?php __('lblNotifications'); ?></b></td>
												<td>
													<select name="admin_sms_notify" id="admin_sms_notify" class="pj-form-field w300">
														<?php
														foreach ($client_notify_arr as $k => $v)
														{
															?><option value="<?php echo ucfirst($k); ?>"><?php echo $v; ?></option><?php
											...
											for ($i = 0; $i < $count; $i++)
											{
												if (($tpl['arr'][$i]['tab_id'] == 6 && (int) $tpl['arr'][$i]['is_visible'] === 1 && strpos($tpl['arr'][$i]['key'], 'admin') >-1 && (strpos($tpl['arr'][$i]['key'], 'sms') > -1)))
												{											
													$rowClass = NULL;
													$rowStyle = "display: none";
